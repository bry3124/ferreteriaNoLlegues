...
const getUsuarioById = (idUsuario) =>
    usuarios.find(user => user.idUsuario === idUsuario);
...
const updateUsuario = (idUsuario, data) => {
    const index = usuarios.findIndex(user => user.idUsuario === idUsuario);
...
const deleteUsuario = (idUsuario) => {
    const index = usuarios.findIndex(user => user.usuarios === idUsuario);
    if (index === -1) return null;
